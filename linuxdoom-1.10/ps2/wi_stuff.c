...
void WI_updateDeathmatchStats(void)
{
	...
	S_StartSound(0, sfx_barexp, MAX_SOUND_COUNT);
	dm_state = 4;
    }
    ...
    {
	if (!(bcnt&3))
	    S_StartSound(0, sfx_pistol, MAX_SOUND_COUNT);
	...
	if (acceleratestage)
	{
	    S_StartSound(0, sfx_slop, MAX_SOUND_COUNT);
...
void WI_updateNetgameStats(void)
{
	...
	if (!stillticking)
	{
	    S_StartSound(0, sfx_pldeth, MAX_SOUND_COUNT);
	    ng_state++;
	}
    }
    else if (ng_state == 10)
	...
	if (acceleratestage)
	{
	    S_StartSound(0, sfx_sgcock, MAX_SOUND_COUNT);
	    if ( gamemode == commercial )
		WI_initNoState();
	    else
		WI_initShowNextLoc();
