...
MusicPS2 playingMusic;
typedef struct music_cache_t
{
    MusicPS2 handle;
    int useful;
} MusicCache;
#define CACHESIZE 12
MusicCache musicCache[CACHESIZE];
int cachesize = 0;
            ...
            int musicVolume)
{
    ...
    memset(musicCache, 0, sizeof(musicCache));
}
...
int CheckCache(int m)
{
    for (int i = 0; i<CACHESIZE; i++)
    {
        if (musicCache[i].useful == 0)
            break;
        
        if (musicCache[i].handle.playing == m)
        {
           // DEBUGLOG("Cache hit");
            musicCache->useful++;
            return i;
...
int InsertCache(int m, tml_message *midi)
{
    int index = -1, min = INT_MAX;
    for (int i = 0; i<CACHESIZE; i++)
    {
        int useful = musicCache[i].useful;
        if (useful == 0 && musicCache[i].handle.midimessage == NULL)
        {
          //  DEBUGLOG("Cache append");
            musicCache[i].useful = 1;
            musicCache[i].handle.midimessage = midi;
            musicCache[i].handle.playing = m;
            return 0;
        }
    ...
    //DEBUGLOG("Cache replace");
    free(musicCache[index].handle.midimessage);
    musicCache[index].useful = 1;
    musicCache[index].handle.midimessage = midi;
    musicCache[index].handle.playing = m;
    return 1;
}
                   ...
                   int looping)
{
    ...
    if (exists != -1)
    {
        playingMusic.playing = musicnum;
        playingMusic.midimessage = musicCache[exists].handle.midimessage;
   ...
   // free(playingMusic.midimessage);
    ...
    unsigned int size = W_LumpLength(music->lumpnum);
    music->handle = I_RegisterSong(music->data);
    playingMusic.playing = musicnum;
    playingMusic.midimessage = tml_load_memory(music->data, size); 
