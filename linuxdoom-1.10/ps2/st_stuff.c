...
boolean ST_checkCheat(char *stringinput)
{
	...
	if (!netgame)
	{
		...
		// 'dqd' cheat for toggleable god mode
		if (cht_compareCheat(&cheat_god, stringinput))
		{
			plyr->cheats ^= CF_GODMODE;
			if (plyr->cheats & CF_GODMODE)
			{
		...
		// 'fa' cheat for killer fucking arsenal
		else if (cht_compareCheat(&cheat_ammonokey, stringinput))
		{
			plyr->armorpoints = 200;
			plyr->armortype = 2;
		...
		// 'kfa' cheat for key full ammo
		else if (cht_compareCheat(&cheat_ammo, stringinput))
		{
			plyr->armorpoints = 200;
			plyr->armortype = 2;
		...
		// 'mus' cheat for changing music
		else if (cht_compareCheat(&cheat_mus, stringinput))
		{
		...
		// Simplified, accepting both "noclip" and "idspispopd".
		// no clipping mode cheat
		else if (cht_compareCheat(&cheat_noclip, stringinput) || cht_compareCheat(&cheat_commercial_noclip, stringinput))
		{
			plyr->cheats ^= CF_NOCLIP;
		...
		for (i = 0; i < 6; i++)
		{
			if (cht_compareCheat(&cheat_powerup[i], stringinput))
			{
				if (!plyr->powers[i])
					P_GivePower(plyr, i);
				else if (i != pw_strength)
		...
		// 'behold' power-up menu
		if (cht_compareCheat(&cheat_powerup[6], stringinput))
		{
			plyr->message = STSTR_BEHOLD;
		}
		// 'choppers' invulnerability & chainsaw
		else if (cht_compareCheat(&cheat_choppers, stringinput))
		{
			plyr->weaponowned[wp_chainsaw] = true;
			plyr->powers[pw_invulnerability] = true;
			plyr->message = STSTR_CHOPPERS;
		...
		// 'mypos' for player position
		else if (cht_compareCheat(&cheat_mypos, stringinput))
		{
			static char buf[ST_MSGWIDTH];
			sprintf(buf, "ang=0x%x;x,y=(0x%x,0x%x)",
					players[consoleplayer].mo->angle,
